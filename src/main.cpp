...
int main(int argc, char *argv[])
{
    ...
    if(argc <= 2)
    {
        const char * cfg_fname = config_filename;
        if(argc == 2)
        {
            cfg_fname = argv[1];
        }
        trace_log_printf(LOG_DEBUG, "using config file %s", cfg_fname);
        cfg_opt_t opts[] = {
    ...
    {
        trace_log_printf(LOG_ERROR, "usage: %s <config_file>", argv[0]);
    }
